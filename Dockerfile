...
RUN CGO_ENABLED=1 GOOS=linux go build -ldflags="-s -w -X main.version=1.0.0 -linkmode external -extldflags -static" -trimpath -o app /usr/app/cmd/*
...
LABEL maintainer="Alexandre Saison <alexandre.saison@gmail.com>" \
  description="Munchin API - Production-ready Go application" \
  version="1.0.0"
